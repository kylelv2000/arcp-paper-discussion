...
from arcp.models import GRADE_OTHER, Member, MeetingConfig, EmailRecipient, Paper
...
def _ensure_columns(app):
    ...
    if 'archived' not in columns:
        ...
        db.session.commit()
    if 'last_grade_update_year' not in columns:
        db.session.execute(text(
            'ALTER TABLE member ADD COLUMN last_grade_update_year INTEGER'
        ))
        db.session.commit()
    ...
    db.session.execute(text("UPDATE member SET grade='9' WHERE grade='other'"))
    # 其余非 0~9 的非法值统一回落为 1 年级
    db.session.execute(text("UPDATE member SET grade='1' WHERE grade NOT GLOB '[0-9]'"))
    db.session.commit()
